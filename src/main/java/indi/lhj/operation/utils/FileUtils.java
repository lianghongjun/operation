package indi.lhj.operation.utils;

import indi.lhj.operation.exception.OperandException;
...
public class FileUtils {
    ...
    public static List<Float> readAsAnswer(String path) {
        ...
        try {
            // 除去序号
            lines = Files.readAllLines(Paths.get(path)).stream()
                    .map(s -> Float.parseFloat(s.substring(s.indexOf("、") + 1))).toList();
